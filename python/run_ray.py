...
# Test 6: BnB with synchronous bound updates from C++ (NEW)
if 6 in tests_to_run:
    print("=== Test 6: BnB z synchronicznymi aktualizacjami ograniczenia z C++ (NOWY) ===")
    start_time = time.time()
    bound_tracker = BoundTracker.remote(int(cost))
    # Using single city tasks with synchronous callbacks
    futures = [solve_city_with_sync.remote(dist, C, i, 1, int(cost), bound_tracker, 1000, 100.0)
               for i in range(1, n)]
    preparing_time = time.time()-start_time
    computing_start_time = time.time()
    results = ray.get(futures)
    ...
    with open(csv_file, mode="a", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([n, C, "Test 6: BnB z synchronicznymi aktualizacjami ograniczenia z C++ (NOWY)", min(results), end_time, preparing_time, computing_time, ct])
